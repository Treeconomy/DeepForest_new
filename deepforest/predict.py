...
def _predict_image_(model,
    ...
    """
    with torch.no_grad():
        print('Predicting image', image.shape)
    ...
    if return_plot:
        # If the input image is provided as a path, load it here to avoid any modifications
        if path:
            original_image = np.array(Image.open(path))
            print('Original image', original_image.shape)
        else:
            original_image = np.array(image.permute(1, 2, 0))
            print('Original image else', original_image.shape)

        # Plot predictions on the original image without any modifications
        img = visualize.plot_predictions(original_image, df, color=color, thickness=thickness)
